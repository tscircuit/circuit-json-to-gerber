...
test("through pad trace route points do not emit invalid copper coordinates", () => {
  const circuitJsonWithThroughPad = [
    {
      type: "pcb_board",
      pcb_board_id: "pcb_board_0",
      center: { x: 0, y: 0 },
    ...
    {
      type: "pcb_trace",
      pcb_trace_id: "trace_with_through_pad",
      route: [
        {
          route_type: "wire",
          x: -4,
  ...
  const gerberOutput = stringifyGerberCommandLayers(
    convertSoupToGerberCommands(circuitJsonWithThroughPad),
  )

  expect(`${gerberOutput.F_Cu}\n${gerberOutput.B_Cu}`).not.toContain("NaN")
  expect(`${gerberOutput.F_Cu}\n${gerberOutput.B_Cu}`).not.toContain(
    "undefined",
  )
  expect(gerberOutput.F_Cu).toContain("X-04000000Y000000000D02*")
  expect(gerberOutput.F_Cu).toContain("X000000000Y000000000D01*")
  expect(gerberOutput.B_Cu).toContain("X000000000Y000000000D02*")
  expect(gerberOutput.B_Cu).toContain("X004000000Y000000000D01*")
})
